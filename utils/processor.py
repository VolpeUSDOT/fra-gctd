...
def _should_crop(frame_width, frame_height, do_crop, crop_width, crop_height,
                crop_x, crop_y):
  if do_crop:
    if all(
...
def process_video(
    ...
    model_path, node_name_map, gpu_memory_fraction, should_crop, crop_width,
    crop_height, crop_x, crop_y, do_deinterlace, num_channels,
    batch_size, clip_length, do_smooth_probs, smoothing_factor, do_binarize_probs):
  ...
  try:
    ...
    frame_width, frame_height, num_video_frames = IO.get_video_dimensions(
  ...
  try:
    should_crop = _should_crop(frame_width, frame_height, should_crop,
                               crop_width, crop_height, crop_x, crop_y)
  except Exception as e:
    logging.error(e)
  ...
  clip_shape = [clip_length, frame_height, frame_width, num_channels]

  logging.debug('FFmpeg output frame shape == {}'.format(clip_shape))
  ...
  analyzer = VideoAnalyzer(
    clip_shape, num_video_frames, len(class_name_map), batch_size, model_input_size,
    model_path, device_type, logical_device_count, os.cpu_count(),
    node_name_map, gpu_memory_fraction, should_crop, crop_x, crop_y,
  ...
  try:
    ...
    num_analyzed_frames, probability_array = result_queue.get()
    ...
    # if num_analyzed_frames != num_video_frames:
    #   if interrupt_queue.empty():
    #     raise AssertionError('num_analyzed_frames ({}) != num_frames '
    #                          '({})'.format(num_analyzed_frames, num_video_frames))
    #   else:
